...
async function doWithPage(page) {
  ...
  for (let gcodeFile of gcodeFiles) {
    ...
    const outputFileName = outputDirName + gcodeFile.replace(inputDirName, "").replace('.gcode', '.png')
    fs.mkdirSync(outputFileName.split('/').slice(0, -1).join('/'), { recursive: true })
    fs.writeFileSync(outputFileName, canvasDataUrl.replace(/^data:image\/png;base64,/, ""), 'base64');
  }
}
